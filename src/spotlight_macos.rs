...
impl SpotlightManager {
    ...
    pub fn init_spotlight_window(&self, window: &Window<Wry>) -> Result<(), Error> {
        ...
        if !registered {
            ...
            handle_focus_state_change(&window, &window_config);
...
fn handle_focus_state_change(window: &Window<Wry>, window_config: &WindowConfig) {
    let auto_hide = match window_config.auto_hide {
